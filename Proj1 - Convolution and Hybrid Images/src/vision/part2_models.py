...
class HybridImageModel(nn.Module):
    ...
    def get_kernel(self, cutoff_frequency: int) -> torch.Tensor:
        ...
        ############################
        ### TODO: YOUR CODE HERE ###
        gaussian_kernel = create_Gaussian_kernel_2D(cutoff_frequency)
        # `np.tile` constructs an array by repeating `gaussian_kernel`
        # n_channels times along the first axis, and once along the
        # remaining axes
        kernel = torch.Tensor(np.tile(gaussian_kernel, (self.n_channels, 1, 1, 1)))
        return kernel
    ...
    def low_pass(self, x: torch.Tensor, kernel: torch.Tensor):
        ...
        ############################
        ### TODO: YOUR CODE HERE ###
        # Read about `conv2d` here: https://docs.pytorch.org/docs/stable/generated/torch.nn.functional.conv2d.html
        filtered_image = F.conv2d(x, kernel, bias=None, padding='same', groups=self.n_channels)
    ...
    def forward(
        ...
        # Need to clamp values to be between 0 and 1 for hybrid image

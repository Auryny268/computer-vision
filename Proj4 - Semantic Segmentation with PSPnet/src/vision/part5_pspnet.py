...
# # Auxiliary loss -> implement it to train off auxiliary
# # Five line change (alledgedly)
...
class PSPNet(nn.Module):
    ...
    def __init__(
        ...
        # to the classifier
        ###########################################################################
        # Number of ResNet50 output feature maps is 2048
        fea_dim = 2048
        if self.use_ppm:
    ...
    def __replace_conv_with_dilated_conv(self):
        ...
        for name, module in self.layer3.named_modules():
            if 'conv2' in name:
                module.stride, module.dilation, module.padding   = 1, 2, 2
            if 'downsample' in name:
                module.stride = 1
        # Adjust Layer 4
        for name, module in self.layer4.named_modules():
            if 'conv2' in name:
                module.stride, module.dilation, module.padding = 1, 4, 4
            if 'downsample' in name:
                module.stride = 1
    ...
    def __create_classifier(self, in_feats: int, out_feats: int, num_classes: int) -> nn.Module:
        ...
        #######################################################################
        cls = nn.Sequential(
            nn.Conv2d(in_feats, out_feats, 3, padding='same'),
    ...
    def forward(
        ...
        comments on zoom_factor: 
            ...
            that the ground truth labels will have already been upsampled by the zoom_factor.
        ...
        h_scaled = int(math.ceil(x_size[2]/8 * self.zoom_factor))
        w_scaled = int(math.ceil(x_size[3]/8 * self.zoom_factor))
        #######################################################################
        # TODO: YOUR CODE HERE                                                #
        #######################################################################
        # Feed the input through the network, upsample the aux output (from layer 3)
        ...
        # Have to initialize aux_loss to a tensor -> if not training aux_loss should be zero
        main_loss, aux_loss = None, None
        # Check self.training == false -> set aux loss to zero tensor

        # F.interpolate automatically resizes height/width instead of batch_size/num_channels if given two-ple (haha)
        aux_logits = F.interpolate(aux_output, size=(h_scaled, w_scaled), mode='bilinear', align_corners=True)
        logits = F.interpolate(output, size=(h_scaled,w_scaled), mode='bilinear', align_corners=True)
        if y != None:
            main_loss  = self.criterion(logits, y)
            if self.training:
                aux_loss = self.criterion(aux_logits, y)
            else:
                aux_loss = torch.tensor([0], dtype=torch.float)

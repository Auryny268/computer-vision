...
def get_critical_indices(model: Union[PointNet, PointNetTNet], pts: torch.Tensor) -> np.ndarray:
    ...
    # Student code begin
    ############################################################################
    # Need to add extra dim to represent batch number
    input_pts = torch.unsqueeze(pts, dim=0)
    _, encodings = model.forward(input_pts)
    crit_pts = torch.argmax(encodings,dim=1)
    crit_indices = torch.unique(crit_pts, dim=1).squeeze()
...
def get_confusion_matrix(
    ...
    # Student code begin
    ############################################################################
    # Just treat it like an iterable -> set of pointclouds, set of labels
    if device == 'cuda':
        model.to(device)

    for ptcloud, labels in loader:
        data = ptcloud.to(device)
        class_outputs, _ = model.forward(data)
        predictions = torch.argmax(class_outputs, dim=1)
        for pred, label in zip(predictions,labels):
            confusion_matrix[label][pred] += 1
    if normalize:
        confusion_matrix = confusion_matrix / np.reshape(np.sum(confusion_matrix, axis=1) , (num_classes,1))

...
class Argoverse(Dataset):
    ...
    def load_path_with_classes(self, split: str, data_root: str) -> Tuple[List[Tuple[str, str]], List[str]]:
        ...
        # Student code begin
        ############################################################################
        class_list = sorted(os.listdir(data_root))
        # pairs = [(os.path.join(data_root, class_), class_) for class_ in class_list]
        for class_ in class_list:
            # Just generate file names
            if split == 'train':
                file_names = [f"{i}.txt" for i in range(170)]
            else:
                file_names =  [f"{i}.txt" for i in range(170,200)]
            pairs += [(os.path.join(data_root, class_, file), class_) for file in file_names]
        # print(class_list)
        # print(pairs)
    ...
    def get_class_dict(self, class_list: List[str]) -> Dict[str, int]:
        ...
        # Student code begin
        ############################################################################
        classes = {c:l for (c,l) in zip(class_list, range(len(class_list)))}
        # raise NotImplementedError(
    ...
    def get_points_from_file(self, path: str) -> torch.Tensor:
        ...
        # Student code begin
        ############################################################################
        a = np.loadtxt(path, dtype=np.float32, skiprows=1)
        pts = torch.from_numpy(a)
    ...
    def pad_points(self, pts: torch.Tensor) -> torch.Tensor:
        ...
        # Student code begin
        ############################################################################
        pts_full = torch.cat((pts, pts[0].repeat(self.pad_size - pts.shape[0],1)))
        # raise NotImplementedError(
    ...
    def __getitem__(self, i) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # Student code begin
        ############################################################################
        path, class_ = self.instances[i]
        pts = self.pad_points(self.get_points_from_file(path))
        label = torch.tensor(self.class_dict[class_])
    ...
    def __len__(self) -> int:
        ...
        # For each path, class pair in self.instances, we want to extract all txt files 
        # from path and see how many points in each

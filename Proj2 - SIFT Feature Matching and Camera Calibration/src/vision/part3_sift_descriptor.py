...
def get_magnitudes_and_orientations(
    ...
    # TODO: YOUR CODE HERE                                                    #
    ###########################################################################
    # np.arctan2 calculates the element-wise arc tangent of Iy/Ix
...
def get_gradient_histogram_vec_from_patch(
    ...
    # Bins have already been provided
    
    ###########################################################################
    # TODO: YOUR CODE HERE                                                    #
    ###########################################################################

    # Magnitudes + orientations are already provided
    # We need to divide 16x16 patch into 16 4x4 cells
    # Orientations cast votes, weights can be calculated from magnitudes
    # Each window will be of shape (16, 16)
    
    wgh = np.zeros((128,1))  # placeholder
    cell_size_w = window_magnitudes.shape[0] // 4
    cell_size_h = window_magnitudes.shape[1] // 4
    idx = 0
    # Each loop increases i,j by cell_size = 4 -> want to loop thru all 4x4 cells
    for i in range(0, 16, cell_size_w):
        for j in range(0, 16, cell_size_h):
            # Second iteration: index is [0:4, 4:8]
            cell_magnitudes = window_magnitudes[i:i+cell_size_w, j:j+cell_size_h].ravel()
            cell_orientations = window_orientations[i:i+cell_size_w, j:j+cell_size_h].ravel()
            # Takes votes from each vector's angle (no need for projections) and weights votes by magnitudes
            # `hist` will be of shape (8,)
            hist, _ = np.histogram(cell_orientations, bins=bins, weights=cell_magnitudes)
            # Place hist in wgh at the right index
...
def get_feat_vec(
    ...
    # Take subsets of magnitudes + orientations for each window
    l_bound, r_bound = int(round(r-half_width+1)), int(round(r+half_width+1))
    u_bound, d_bound = int(round(c-half_width+1)), int(round(c+half_width+1))
    feature_mag = magnitudes[l_bound:r_bound, u_bound:d_bound]
    feature_orient = orientations[l_bound:r_bound, u_bound:d_bound]
    feature_hist = get_gradient_histogram_vec_from_patch(feature_mag, feature_orient)
    return np.sqrt(feature_hist / np.linalg.norm(feature_hist))
...
def get_SIFT_descriptors(
    ...
    # TODO: YOUR CODE HERE                                                    #
    ###########################################################################
    # Get magnitudes + orientations for each point
    Ix, Iy = compute_image_gradients(image_bw)
    magnitudes, orientations = get_magnitudes_and_orientations(Ix, Iy)
    # For each feature point we want to calculate feature vector
    # First, calculate feat_dim
    k = X.shape[0]
    feat_dim = 8 * ((feature_width // 4) ** 2)
    fvs = np.full([k,feat_dim], np.nan)
    for i in range(k):
        # Need to ravel from (128,1) to (128,), otherwise Python has a conniption
        # Need to throw out points

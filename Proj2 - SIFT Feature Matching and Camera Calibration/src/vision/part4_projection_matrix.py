...
def calculate_projection_matrix(
    ...
    ###########################################################################
    """
    Linear Akgebra Refresher:
    - A is (mxn) matrix where m>n
    - rank(A) = n => Ax-b has unique LSTSQ solution
    - Ax=b solved by normal equations A.TAx = A.Tb
    - In singular values of A are square roots of eigenvalues of A.TA
        - if A is nxn real symmetric, then singular values = eigenvalues

    Goal: Solve for x using either LSTSQ or SVD
    """
    # Idea: Take SVD of assembled matrix -> A=USV.T, where U - mxn, S - nxn, Vt = nxn
    # Using code from lecture slides -> trying to solve for nullspace of A consisting of (1xn) vectors
    # https://math.stackexchange.com/questions/1771013/how-is-the-null-space-related-to-singular-value-decomposition
    # ^ N(A) spanned by V_i, where i = (rho+1,...,n) where rho is # of non-zero singular values
...
def projection(P: np.ndarray, points_3d: np.ndarray) -> np.ndarray:
    ...
    N = points_3d.shape[0]
    homog_points_3d = np.concatenate((points_3d, np.ones((N,1))), axis=1)
    homog_points_2d = (P@(homog_points_3d.T)).T
    # Divide by last coordinate for each point

    # np.tile(np.reshape(Hx[:,2],(N,1)), (1,3)))
    # ^ Create 1d array of N points in the last column of Hx, 
    # and repeat 3 times along axis=1   
    projected_points_2d = np.divide(homog_points_2d, np.tile(np.reshape(homog_points_2d[:,-1],(N,1)), (1,3)))
...
def calculate_camera_center(M: np.ndarray) -> np.ndarray:
    ...
    # TODO: YOUR CODE HERE                                                    #
    ###########################################################################
    # Use np.linalg.solve(a,b) where ax=b
    # cc = -Q^-1m_4 where M = [Q|m4]
    Q, m4 = M[:3,:3], M[:,-1]

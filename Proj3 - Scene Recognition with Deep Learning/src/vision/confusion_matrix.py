...
def generate_confusion_data(
    ...
    # Data loaded in batches of 32
    for batch in loader:
        # Each batch contains inputs and their labels
        images, target_labels = batch[0], batch[1]
        # Must make sure both tensors are on the same device!!!!
        if cuda:
            images, target_labels = images.cuda(), target_labels.cuda()
        # Get predictions using model.forward
        logits = model.forward(images)
        predicted_labels = torch.argmax(logits, dim=1)
        preds.append(predicted_labels)
        targets.append(target_labels)
    preds, targets = torch.cat(preds), torch.cat(targets)
    # Sort labels_to_idx by indices (returns list of tuples) -> extract only the labels
    class_labels = [k for k, _ in sorted(label_to_idx.items(), key=lambda item: item[1])]
    model.train()
...
def generate_confusion_matrix(
    ...
    if normalize:
        actual_instances = np.sum(confusion_matrix, axis=1)
        confusion_matrix = confusion_matrix / np.reshape(actual_instances,(actual_instances.size, 1))

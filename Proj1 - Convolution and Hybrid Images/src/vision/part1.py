...
def create_Gaussian_kernel_1D(ksize: int, sigma: int) -> np.ndarray:
    ...
    # I referenced [this answer](https://stackoverflow.com/questions/29731726/how-to-calculate-a-gaussian-kernel-matrix-efficiently-in-numpy) on stack exchange
    # Bc ksize is always odd, edges should be |ksize - 1|/2
...
def create_Gaussian_kernel_2D(cutoff_frequency: int) -> np.ndarray:
    ...
    return np.outer(create_Gaussian_kernel_1D(k,sigma), create_Gaussian_kernel_1D(k,sigma))
    raise NotImplementedError(
...
def my_conv2d_numpy(image: np.ndarray, filter: np.ndarray) -> np.ndarray:
    ...
    filtered_img = np.zeros((m, n, c))

    # Slide filter over image
    for x in range(m):
      for y in range(n):
        for z in range(c):
          filtered_img[x][y][z] = np.sum(filter * padded_img[x:x+k, y:y+j,z])
    return filtered_img
...
def create_hybrid_image(
    ...
    ############################
    ### TODO: YOUR CODE HERE ###
    # Used the link below to read up on hybrid images and gaussian filters 
    # https://homepages.inf.ed.ac.uk/rbf/HIPR2/gsmooth.htm
    # Idea: Use my_conv2d_numpy to apply the filter to image1 to get low frequencies
    # Step 1: Blur image1 with filter to get low frequencies
    low_frequencies = my_conv2d_numpy(image1, filter)
    # Step 2: Subtract blurred image2 from image2 to get high frequencies
    high_frequencies = image2 - my_conv2d_numpy(image2, filter)
    # Step 3: Add low frequencies and high frequencies to get hybrid image
    hybrid_image = low_frequencies + high_frequencies
    # Step 4: Clip hybrid image to be between 0 and 1 using np.clip

...
def compute_image_gradients(image_bw: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    ...
    # Idea: Sobel filters are already provided, so I can get use them directly
    # Use code from Part 3 of Proj1 -> assumes that image and kernel are already torch tensors
    # `unsqueeze(0)` adds a new dimension of size 1 at the 0th index
    sobelx_tensor = torch.from_numpy(SOBEL_X_KERNEL).unsqueeze(0).unsqueeze(0)
    sobely_tensor = torch.from_numpy(SOBEL_Y_KERNEL).unsqueeze(0).unsqueeze(0)
    # Need `.float()` bc many PyTorch models won't work with integer tensors
    image_tensor = torch.from_numpy(image_bw).unsqueeze(0).unsqueeze(0).float()
    # Use F.conv2d to convolve image with Sobel filters
    Ix = F.conv2d(image_tensor, sobelx_tensor, padding='same').squeeze().numpy()
    Iy = F.conv2d(image_tensor, sobely_tensor, padding='same').squeeze().numpy()
    return Ix, Iy
...
def compute_harris_response_map(
    ...
    # Idea: Use second_moments() to get sx2, sy2, sxsy
    # Create M from these, then compute det(M) and trace(M)
    # Finally compute R = det(M) - alpha * (trace(M))^2
...
def nms_maxpool_pytorch(
    ...
    # Checks for locations where R == R_max (which is an array of local maxima)
    mask = torch.where(R == R_max, torch.ones_like(R), torch.zeros_like(R))
    R = R * mask

    # Confidence scores are the Harris response scores
    scores = R
...
def get_harris_interest_points(
    ...
    # Idea: Use compute_harris_response_map() to get R
    # Normalize R to [0,1]
    # Use nms_maxpool_pytorch() to get top k interest points
    # Use remove_border_vals() to remove points too close to border

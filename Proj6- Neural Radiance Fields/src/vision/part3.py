...
def train_nerf(images, tform_cam2world, cam_intrinsics, testpose, testimg, height, width, 
    ...
    ##########################################################################
    model = NerfModel(encode_channels)
    model.to(device)
    # initializing weights
    # Need to write a function that checks if module layer is linear, and if so then initialize with Xavier Uniform
    def init_weights(m):
        if isinstance(m, nn.Linear):
            torch.nn.init.xavier_uniform_(m.weight)
    # use model.apply with custom function
    ...
    for i in range(num_iters + 1):
        ...
        if i % display_every == 0: 
            ...
            ##########################################################################
            with torch.no_grad():
                rgb_predicted, depth_predicted = render_image_nerf(height, width, cam_intrinsics, testpose, near_thresh, far_thresh, depth_samples_per_ray, encode, model, True)
            # raise NotImplementedError("Implement based on the above instructions.")

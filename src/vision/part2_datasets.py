...
def make_dataset(path: str) -> Tuple[List[str], List[str]]:
    ...
    # `os.path.join(path, file)` Combines the directory path (path) with the filename (file) into a full path string.
...
class HybridImageDataset(data.Dataset):
    ...
    def __init__(self, image_dir: str, cf_file: str) -> None:
        ...
        ############################
        ### TODO: YOUR CODE HERE ###
        # Check that number of images in each set is the same
        assert len(images_a) == len(images_b)
        # Use Image.open() from PIL to read images
        # Convert images from str to PIL Image to Torch Tensor
        for i in range(len(images_a)):
            images_a[i] = self.transform(PIL.Image.open(images_a[i]))
            images_b[i] = self.transform(PIL.Image.open(images_b[i]))
        # End goal:
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor, int]:
        ...
        ############################
        ### TODO: YOUR CODE HERE ###
        return self.images_a[idx], self.images_b[idx], self.cutoff_frequencies[idx]
        # return self.images_a[idx], self.images_b[idx], self.cutoff_frequencies[idx]
        # raise NotImplementedError(

...
def compute_accuracy(logits: torch.Tensor, labels: torch.Tensor) -> float:
    ...
    # Student code begin
    ############################################################################
    # Idea: Iterate thru rows, add to batch accuracy if correct pred
    batch_size, num_classes = logits.shape
    # ChatGPT solution
    # torch.argmax(dim=0) searches each col (across rows)
    # torch.argmax(dim=1) searches each row (across cols)
    # Gets highest index for each row
    predicted_labels = torch.argmax(logits, dim=1)
    # Compare to true labels, .sum() gets correct num of comparisons
    # .item() extracts value of sum from resulting tensor
...
def compute_loss(
    ...
    # Student code begin
    ############################################################################
    # Calculate class probabilities for model output -> Must count along rows
    # prob_output = torch.nn.Softmax(1)(model_output)
    # Create y tensor
    # y = torch.zeros(model_output.shape[1])
    # y[target_labels] = 1
    # Calculate loss -> pass in model output and target labels
    loss = model.loss_criterion(model_output, target_labels)
    # normalize loss
    # if is_normalize:
    #     loss = loss / model_output.shape[0]
    # ^ since nn.NLLLoss() already averages over the batch by default (reduction="mean"). then this line is unnecessary
    return loss
